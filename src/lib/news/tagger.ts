...
// `transfer` was on the Sports list and was firing 🏆 Sports on every story
// involving a "transfer of power", "wire transfer", "data transfer", etc.
// Combined with the substring `.includes()` matching used previously, the
// Sports tag was showing up on Iran war, Brazil politics, and oil earnings.
// Two fixes: drop `transfer` (too broad to disambiguate from soccer transfers
// without context the tagger doesn't have) and switch to word-boundary
// matching everywhere so short keywords like `nba` don't substring-match
...
const TOPIC_TAGS: Record<string, string[]> = {
  ...
  '🏆 Sports':          ['championship', 'nfl', 'nba', 'world cup', 'super bowl', 'playoff', 'premier league', 'olympics', 'ufc'],
...
export function extractTopics(title: string, summary: string): string[] {
  ...
  for (const [tag, keywords] of Object.entries(GEO_TAGS)) {
    if (keywords.some(kw => wordBoundaryMatch(text, kw))) {
      found.push(tag)
      if (found.filter(t => FLAG_PATTERN.test(t)).length >= 2) break
    }
  }
